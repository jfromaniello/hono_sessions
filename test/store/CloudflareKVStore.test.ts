...
import { CloudflareKVStore, KVNamespace } from '../../src/store/CloudflareKVStore';
...
describe('CloudflareKVStore', () => {
  ...
  beforeEach(() => {
    ...
    // Create a fresh KVNamespace mock for each test
    mockKV = createMockKV();
  ...
  describe('constructor', () => {
    ...
    it('should initialize with custom values when provided', () => {
      ...
      expect(customStore['kv']).toBe(mockKV);
  ...
  describe('getSessionById', () => {
    ...
    it('should return null if session is not found in KV', async () => {
      ...
      expect(mockKV.get).toHaveBeenCalledWith('session:test-session-id', { type: 'json' });
    });
    ...
    it('should return null and log error if exception occurs', async () => {
      ...
      expect(result).toBeNull();
      expect(consoleErrorSpy).toHaveBeenCalledWith('Error retrieving session:', expect.any(Error));
      expect(mockKV.get).toHaveBeenCalledWith('session:test-session-id', { type: 'json' });
    });
  });
  ...
  describe('createSession', () => {
    it('should throw error if sessionId is not provided', async () => {
      ...
      await expect(kvStore.createSession('', sessionData)).rejects.toThrow('Session ID is required');
    ...
    it('should log and rethrow error if exception occurs', async () => {
      ...
      await expect(kvStore.createSession('test-session-id', sessionData)).rejects.toThrow('KV error');
  ...
  describe('persistSessionData', () => {
    it('should throw error if sessionId is not provided', async () => {
      ...
      await expect(kvStore.persistSessionData('', sessionData)).rejects.toThrow('Session ID is required');
      expect(mockKV.put).not.toHaveBeenCalled();
    });
    ...
    it('should log and rethrow error if exception occurs', async () => {
      ...
      await expect(kvStore.persistSessionData('test-session-id', sessionData)).rejects.toThrow('KV error');
      expect(consoleErrorSpy).toHaveBeenCalledWith('Error persisting session data:', expect.any(Error));
    });
  });
  
  describe('deleteSession', () => {
    it('should throw error if sessionId is not provided', async () => {
      await expect(kvStore.deleteSession('')).rejects.toThrow('Session ID is required');
    ...
    it('should log and rethrow error if exception occurs', async () => {
      ...
      await expect(kvStore.deleteSession('test-session-id')).rejects.toThrow('KV error');
